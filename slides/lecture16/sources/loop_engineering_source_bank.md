# 第十六讲资料库：Loop Engineering 与业务工作流

本资料库服务于《机器学习与自然语言处理》第十六讲。课堂目标不是重复 Agent、Prompt、Skill、Swarm/Team 的定义，而是把全课已经讲过的组件收束为一个可复核、可迭代、可交付的业务工作流。
...
1. Addy Osmani, "Loop Engineering", 2026.
   ...
   - 转译：不要把这篇文章讲成新名词介绍，而要讲成“prompt 被放进一个能持续运行的业务系统”。
...
2. LangChain, "The Art of Loop Engineering", 2026.
   ...
   - 转译：企业项目最缺的往往不是 agent loop，而是 verification loop、guardrail、handoff 和停止条件。
...
3. OpenAI Cookbook, "Agent Improvement Loop".
   ...
   - 转译：学生期末项目也可以照这个逻辑写：先保存运行记录，再用人工/模型评分找失败，再修改业务规则、工具和流程。
...
5. OpenAI, "A practical guide to building agents".
   ...
   - 课堂用途：给业界落地框架。重点包括 workflow、tools、guardrails、handoffs、single-agent 与 multi-agent 的取舍。
   - 转译：最后一讲要把 agent 从“聊天助手”转成“业务流程执行者”：它读什么数据、能调用什么工具、何时拒绝、何时升级人工。
...
6. Anthropic, "How we built our multi-agent research system".
   ...
   - 转译：第十二讲已经讲过 Swarm/Team，本讲只用它说明“loop 需要状态、共享记忆、成本约束和汇聚规则”。
...
> 我把期末项目设计成了一个可检查的 AI 业务工作流：输入是什么，Agent 做什么，产物是什么，谁来检查，什么情况下重做，什么情况下停止或人工接管。
